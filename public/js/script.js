...
function retPackage1(abj1){
	setTimeout(function(){
		document.getElementById('area-list-container').removeAttribute('style');
	document.getElementById('area-list-container').innerHTML="";
	abj1.value="";
},500);
	
}
function retPackage(abj){
	var packageName=abj.value;
//alert(packageName);
	if(packageName==''){
		document.getElementById('area-list-container').removeAttribute('style');
		document.getElementById('area-list-container').innerHTML='';
	...
	var xhr3;  
        ...
        var datastring3='area=' + packageName;
//alert(datastring3);
         function display_data3() {  	
                     if (xhr3.readyState == 4) {  
                      if (xhr3.status == 200) {
                      	//alert(xhr3.responseText);
	                      	// $('#area-list-container').removeClass('area-list-container').addClass('area-list-container1');
	                        document.getElementById('area-list-container').innerHTML=xhr3.responseText;
	                        document.getElementById('area-list-container').setAttribute('style','height:200px;');
	                        if (xhr3.responseText="") {
	                        	document.getElementById('area-list-container').removeAttribute('style');
	                        }
	                        //document.getElementById('area').setAttribute('style','border:1px solid #fff');
                        // }
                        // else{
                        // 	alert('else'+xhr3.responseText);
                        // 	document.getElementById('area-list-container').removeAttribute('style');
            ...
            xhr3.open("POST", "retPackage.php", true);   
...
//alert(place);
	if(place==''){ 
		$('#place').addClass('your-class');
		err=1;
	};
	// if(date1==''){
	// 	$('#date1').addClass('your-class');
	// 	err=1;
	// };
	// if(duration==''){
	// 	$('#duration').addClass('your-class');
	// 	err=1;
	// };
	if(mobile==''){
		errcode=1;
	...
	if(err==1 && errcode==2){
		...
		$('#emailid').addClass('your-class');
		document.getElementById('myerrid').innerHTML='Kindly enter your Email Id or Mobile Number!';
		$('#myerrid').fadeIn();
	}else if (err==1) {
		$('#mobile').removeClass('your-class');
		$('#emailid').removeClass('your-class');
		document.getElementById('myerrid').innerHTML='Kindly let us know your desired destination!';
		$('#myerrid').fadeIn();
	}else{
		$('#myerrid').fadeOut();
		document.getElementById('myerrid').innerHTML='';
        ...
        var datastring4='place=' + place+'&date1='+date1+'&duration='+duration+'&mobile='+mobile+'&emailid='+emailid;
//alert(datastring4);
         function display_data5() {  	
                     if (xhr4.readyState == 4) {  
                      if (xhr4.status == 200) {
	                     if(xhr4.responseText=='done'){
	                     	document.getElementById('submitButton').innerHTML='Sent Successfully!';
							...
							document.getElementById('emailid').value='';
							$('#place').removeClass('your-class');
							$('#date1').removeClass('your-class');
							$('#duration').removeClass('your-class');
							$('#mobile').removeClass('your-class');
							$('#emailid').removeClass('your-class');
							setTimeout(function(){
								document.getElementById('submitButton').innerHTML='Get A Call Back';
							},3500);
	                     }else{
            ...
            xhr4.open("POST", "saveDetails.php", true);   
            xhr4.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");                    
            xhr4.send(datastring4);  
            xhr4.onreadystatechange = display_data5; 
	}
}
...
function subscribe(){
    ...
    if(err==1){
        ...
        var datastring2='subscribeName=' + subscribeName;
// alert(datastring2);
         function display_data2() {     
                     if (xhr2.readyState == 4) {  
                      if (xhr2.status == 200) {
                        if (xhr2.responseText=="done") {
                            document.getElementById('subscribeName').value='';
                            document.getElementById('subscribeName').setAttribute('placeholder','Subscribed Successfully');
                        }else{
                            document.getElementById('subscribeName').value='';
                            document.getElementById('subscribeName').setAttribute('placeholder','Try again letter');
                        }
                      }
                      else{
            ...
            xhr2.open("POST", "saveSubcribeData.php", true);   
            xhr2.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");                    
            xhr2.send(datastring2);  
            xhr2.onreadystatechange = display_data2; 
    }
}
...
function checkFile(fieldObj){
  document.getElementById('selectfile1').setAttribute("style","border:1px solid #ccc;");
  var FileName=fieldObj.value;var FileExt=FileName.substr(FileName.lastIndexOf('.')+1);
  var FileSize=fieldObj.files[0].size;
  var FileSizeKB=FileSize*0.001;
    if((FileExt!="jpg"&&FileExt!="png"&&FileExt!="jpeg")||FileSizeKB>300)
    {
      var error="File type : "+FileExt+"<br />";
      error+="Size: "+FileSizeKB+" KB <br />";
      error+="Please make sure your file is in .jpg, .jpeg or .png format and less than 300 KB.\n\n";
      alert(error);fieldObj.value="";
    }
    else{
      $('#fileli1 label').html('DONE');
      document.getElementById('fileli1').setAttribute("style","border:1px solid #ccc; background-color:green;color:#fff;");
      $('#fileli1 label').css("color","#fff");
    };
...
function testimonial(){
	...
	var elength=email1.length;
	var cno1=document.getElementById('cno1').value;
	var feedback=document.getElementById('feedback').value;
	var image=document.getElementById('selectfile1').value;
    var Ferr=0;
    var errcode=0;
    var Errormsg='';
    if(name1==''){
    	document.getElementById('name1').setAttribute('style',"border:1px solid red");
    	Ferr=1;
    };
    if(feedback==''){
    	document.getElementById('feedback').setAttribute('style',"border:1px solid red");
    	Ferr=1;
    };
    if((cno1!='')||(email1.indexOf("@")>1 || email1.lastIndexOf(".")>email1.indexOf("@")+2 || email1.length>email1.lastIndexOf(".")+1)){
		
	}else{
		document.getElementById('email1').setAttribute('style',"border:1px solid red");
		document.getElementById('cno1').setAttribute('style',"border:1px solid red");
		Ferr=1;
		errcode=1;
	}
	if(Ferr==1 && errcode==1){
		document.getElementById('Ferr').innerHTML='Kindly enter your Email Id or Mobile Number!';
		document.getElementById('Ferr').setAttribute('style',"color:red");
    	$('#Ferr').slideDown();
    	setTimeout(function(){
    		$('#Ferr').slideUp();
    	},3000);
	}else if (Ferr==1) {
		document.getElementById('Ferr').innerHTML='Please fill all the details properly!';
		document.getElementById('Ferr').setAttribute('style',"color:red");
    	$('#Ferr').slideDown();
    	setTimeout(function(){
    		$('#Ferr').slideUp();
      ...
      m_data.append('file1',$('input[name=image]')[0].files[0]);
      m_data.append('name',name1);
      m_data.append('email',email1);
      m_data.append('cno',cno1);
      m_data.append('feedback',feedback);

     $.ajax({url:'feedback.php',data:m_data,processData:false,contentType:false,type:'POST',dataType:'json',success:function(response){
//alert(response.text);
            if (response.type=='error') {
              document.getElementById('Ferr').setAttribute('style','color:red');
              document.getElementById('Ferr').innerHTML=response.text;
              $('#Ferr').slideDown();
              setTimeout(function(){
               $('#Ferr').slideUp();
              ...
              document.getElementById('Ferr').setAttribute('style','color:green');
              document.getElementById('Ferr').innerHTML=response.text;
              $('#Ferr').slideDown();
              setTimeout(function(){
               $('#Ferr').slideUp(); 
...
function limitlength(lmtobj, length){
		$('#myerrid').fadeOut();
		document.getElementById('myerrid').innerHTML='';    
      $('#mobile').removeClass('your-class');
		$('#emailid').removeClass('your-class');
            ...
            if (maxlength==14) {
                var chk=isNaN(lmtobj.value);
                if (lmtobj.value.length>maxlength){
                    lmtobj.value=lmtobj.value.substring(0, maxlength);
                }
                else if(chk==true){
                    lmtobj.value=lmtobj.value.substring(0, lmtobj.value.length-1);
                };
            }
           else if (lmtobj.value.length>maxlength){
           	$('#place').removeClass('your-class');
            lmtobj.value=lmtobj.value.substring(0, maxlength);
            lmtobj.focus();
            };
...
function limitlength1(lmtobj, length){
		document.getElementById('name1').setAttribute("style","border:1px solid #ccc;");
		document.getElementById('email1').setAttribute("style","border:1px solid #ccc;");
		document.getElementById('cno1').setAttribute("style","border:1px solid #ccc;");
		document.getElementById('feedback').setAttribute("style","border:1px solid #ccc;");
